...
export interface HazardEvent {
  ...
  limitations: string[];
  // Rich extension fields for frontend visualization & EAP context
  label?: string;
...
export const CANONICAL_ACTION_STATUS_ORDER: ActionStatus[] = [
  "drafted",
  "awaiting_approval",
  "approved",
  "acknowledged",
...
export interface Action {
  ...
  // Rich extension fields for role navigation & frontend board
  description?: string;
...
export interface BudgetProject {
  id: string;
  name: string;
  cost: number; // Cost in lakhs (integer/decimal) or integer currency units
  costLakhs?: number;
  criticality: number; // 0 - 10 scale
  criticalityScore?: number;
  populationImpact: number; // Population reach count
...
export interface OptimiseRequest {
  ...
  budget: number; // Budget in lakhs or integer rupees
  budgetLakhs?: number;
}
